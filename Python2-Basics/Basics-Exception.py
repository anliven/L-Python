...
except ShortInputException as x:  # 提供了错误类和用来表示错误/异常对象的变量。这与函数调用中的形参和实参概念类似。
...
def temp_convert(var):
    ...
    try:
        print (int(var))
    except ValueError as Argument:  # 一个异常可以带上参数来输出异常信息。通过except语句来捕获异常的参数。
...
except NetworkWrongError as e:  # 在这里，变量 e 是NetworkWrongError类的实例。
    print (e.arg)
