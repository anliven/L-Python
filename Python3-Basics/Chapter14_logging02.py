...
file_handler = logging.FileHandler('Chapter14_logging02_output.log')  # 创建FileHandler对象
...
# ### 基本概念
# - Logger：日志记录器，应用程序中可以直接使用的接口；
# - Handler：日志处理器，用以表明将日志保存到什么地方以及保存多久；
# - Formatter：格式化，用以配置日志的输出格式；
# 上述三者的关系是：一个Logger使用一个Handler，一个Handler使用一个Formatter；
#
# ### Handler（日志记录器）
