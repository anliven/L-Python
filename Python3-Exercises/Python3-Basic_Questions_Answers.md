...
多用在编写代码时，如果只是暂时编写了函数声明，使用pass语句可以保证语法正确。
```
...
### 052 - 如何实现多线程？
```
Python内置有多线程模块，例如multiprocessing；
但由于GIL的限制，Python并不支持真正意义上的多线程；
GIL（全局解释器锁）会确保任何时候，多个线程中只有一个线程被执行；
一个线程保存GIL并在将其传递给下个线程之前执行一些操作，也就是说实际上只是线程在CPU 上轮流运行；
```
### 053 - 类的继承有几种场景？
```
...
- 解释器管理这个私有堆空间，开发者无法访问它，但可以通过核心API访问一些工具；
- Python内存管理器控制内存分配；
- 内置垃圾回收器会回收使用所有未使用内存，使其适用于堆空间；
```
